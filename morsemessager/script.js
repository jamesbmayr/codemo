	...
	/* encodeText */
		...
		function encodeText(event) {
			try {
				...
				// split & convert
					var characters = plaintext.replace(/\n/g, "⏎").split("")
					for (var i in characters) {
						characters[i] = CONSTANTS.textToMorse[characters[i].toLowerCase()] || "?"
					}

				// rejoin & show
					ELEMENTS.codetext.value = characters.join(" ").replace(/\s?⏎\s?/g, "\n")
			}
	...
	/* decodeText */
		...
		function decodeText(event) {
			try {
				...
				// rejoin & show
					ELEMENTS.plaintext.value = characters.join("").replace(/\s?⏎\s?/g, "\n")
			}
			catch (error) { console.log(error) }
		}
	...
	/* storeAsset */
		window.ASSETS_J.storeAsset = async function(type) {
			try {
				// txt
					return {
						name: "morseMessager_" + (new Date().getTime()) + ".txt",
						type: "txt",
						data: ELEMENTS.plaintext.value + "\n\n" + ELEMENTS.codetext.value
					}
			} catch (error) {console.log(error)}
		}
