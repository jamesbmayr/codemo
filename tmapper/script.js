	...
	/*** globals ***/
		...
		/* constants */
			var CONSTANTS = {
				...
				geoOffsetLatitude: 42.31550913644465, // y
		...
		/* API */
			var API = {
				...
				lines: {
					...
					"Green-C": {id: "Green-C", color: "#00843D", display: "Green Line C", stops: ["Lechmere", "Haymarket", "Government Center", "Park Street", "Boylston", "Arlington", "Copley", "Hynes Convention Center", "Kenmore", "Saint Mary's Street", "Hawes Street", "Kent Street", "Saint Paul Street (C)", "Coolidge Corner", "Summit Avenue", "Brandon Hall", "Fairbanks Street", "Washington Square", "Tappan Street", "Dean Road", "Englewood Avenue", "Cleveland Circle"]},
					"Green-D": {id: "Green-D", color: "#00843D", display: "Green Line D", stops: ["Lechmere", "Haymarket", "Government Center", "Park Street", "Boylston", "Arlington", "Copley", "Hynes Convention Center", "Kenmore", "Fenway", "Longwood", "Brookline Village", "Brookline Hills", "Beaconsfield", "Reservoir", "Chestnut Hill", "Newton Centre", "Newton Highlands", "Eliot", "Waban", "Woodland", "Riverside"]},
					"Green-E": {id: "Green-E", color: "#00843D", display: "Green Line E", stops: ["Lechmere", "Haymarket", "Government Center", "Park Street", "Boylston", "Arlington", "Copley", "Prudential", "Symphony", "Northeastern University", "Museum of Fine Arts", "Longwood Medical Area", "Brigham Circle", "Fenwood Road", "Mission Park", "Riverway", "Back of the Hill", "Heath Street"]},
			...
			function grabMap(event) {
				try {
					...
					// set grab position
						...
						ELEMENTS.grabbing = {
							x: cursor.x - ELEMENTS.offset.x,
							y: cursor.y - ELEMENTS.offset.y
						}
		...
		/* moveMap */
			...
			function moveMap(event) {
				try {
					...
					// set center
						ELEMENTS.offset.x = cursor.x - ELEMENTS.grabbing.x
						ELEMENTS.offset.y = cursor.y - ELEMENTS.grabbing.y
		...
		/* zoomCanvas */
			...
			function zoomCanvas(event) {
				try {
					// get scroll amount
						if (event.wheelDelta) {
							var modifier = (event.wheelDelta > 0) ? CONSTANTS.zoomStep : -CONSTANTS.zoomStep
						}
						else if (event.deltaY) {
							var modifier = (event.deltaY < 0) ? CONSTANTS.zoomStep : -CONSTANTS.zoomStep
						}

					// update zoom
						ELEMENTS.offset.z = Math.max(CONSTANTS.zoomMin, ((ELEMENTS.offset.z * 1000) + (modifier * 1000)) / 1000)
	...
	/*** API ***/
						...
						var request = new XMLHttpRequest()
							...
							request.onload = function() {
								if (request.readyState === XMLHttpRequest.DONE && request.status === 200) {
									API.vehicles = JSON.parse(request.responseText).data
								}
							}
							request.send()
				} catch (error) {console.log(error)}
		...
		/* dipslayLines */
			function displayLines() {
				try {
					// loop through lines
						for (var i in API.lines) {
							// loop through stops
								var line = API.lines[i]
								for (var j = 1; j < line.stops.length; j++) {
									// get stops
										var fromStop = API.stops[line.stops[j - 1]] || null
										var toStop   = API.stops[line.stops[j]    ] || null
		...
		/* displayStops */
			function displayStops() {
				try {
					// loop through stops
						for (var i in API.stops) {
							// get stop coordinates
								var x = (API.stops[i].longitude * CONSTANTS.geoZoom * ELEMENTS.offset.z) - (CONSTANTS.geoOffsetLongitude 	* CONSTANTS.geoZoom * ELEMENTS.offset.z) 
								var y = (API.stops[i].latitude 	* CONSTANTS.geoZoom * ELEMENTS.offset.z) - (CONSTANTS.geoOffsetLatitude 	* CONSTANTS.geoZoom * ELEMENTS.offset.z)
			...
			function displayVehicles() {
				try {
					// loop through vehicles
						for (var i in API.vehicles) {
							// get vehicle coordinates
								var vehicle = API.vehicles[i]
								var x = (vehicle.attributes.longitude 	* CONSTANTS.geoZoom * ELEMENTS.offset.z) - (CONSTANTS.geoOffsetLongitude 	* CONSTANTS.geoZoom * ELEMENTS.offset.z) 
								var y = (vehicle.attributes.latitude 	* CONSTANTS.geoZoom * ELEMENTS.offset.z) - (CONSTANTS.geoOffsetLatitude 	* CONSTANTS.geoZoom * ELEMENTS.offset.z)
								...
								if (vehicle.relationships && vehicle.relationships.route && vehicle.relationships.route.data && vehicle.relationships.route.data.id) {
									var color = API.lines[vehicle.relationships.route.data.id] ? API.lines[vehicle.relationships.route.data.id].color : CONSTANTS["dark-gray"]
								}
