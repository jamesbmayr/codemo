		...
		window.ASSETS_J.storeAsset = async function(type) {
			try {
				// json
					if (type == "json") {
						return {
							name: "xmlParser_" + (new Date().getTime()) + ".json",
							type: "json",
							data: document.getElementById("output-json").innerText
						}
					}
				...
				// html
					if (type == "html") {
						return {
							name: "xmlParser_" + (new Date().getTime()) + ".html",
							type: "html",
							data: "<style>details { margin-left: 20px; }</style>" + document.getElementById("output-tree").innerHTML
						}
					}
			} catch (error) {console.log(error)}
		}
