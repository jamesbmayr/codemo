...
/*** changes ***/
	/* changeInstrument */
		...
		function changeInstrument() {
			...
			var name = document.getElementById("wave").value.trim()
	...
	/* changeVolume */
		...
		function changeVolume() {
			...
			if (AUDIO_J.instruments[AUDIO_J.activeInstrumentId]) {
				AUDIO_J.instruments[AUDIO_J.activeInstrumentId].setParameters({ volume: (volume / 100) })
			}
		}
...
/* playNote */
	...
	function playNote(pitch) {
		//sound
			...
			var frequency = AUDIO_J.getNote(pitch)[0]

			if (AUDIO_J.instruments[AUDIO_J.activeInstrumentId]) {
				AUDIO_J.instruments[AUDIO_J.activeInstrumentId].press(frequency)
				AUDIO_J.instruments[AUDIO_J.activeInstrumentId].lift(frequency, (60 * 1000 / tempo))
			}
						...
						if (pitches.length > 0) {
							for (var p in pitches) {
								var data = pitches[p].id.split("-")
									var pitch = data[5]
									var array = AUDIO_J.getNote(pitch)
