...
/*** data ***/
	/* clearData */
		...
		function clearData() {
			if (Object.keys(flagData).length) {
				previous.push(JSON.stringify(flagData))
			}
	...
	/* previousData */
		...
		function previousData() {
			if (previous.length) {
				flagData = JSON.parse(previous[previous.length - 1])
				previous.pop()
	...
	/* randomizeData */
		...
		function randomizeData() {
			...
			// field
				flagData.fieldHue = chooseRandom(hues)
				flagData.fieldShade = flagData.fieldHue[1]
				flagData.fieldHue = flagData.fieldHue[0]

			// structure
				flagData.structure = chooseRandom(structures)
				flagData.sectionCount = (flagData.structure == "solid") ? 1 : flagData.structure.includes("stripes") ? (Math.floor(Math.random() * 7) + 2) * (flagData.structure == "wedge-stripes" ? 2 : 1) : (flagData.structure == "checkers") ? Math.pow((Math.floor(Math.random() * 6) + 2), 2) : (Math.floor(Math.random() * 4) + 1)
				flagData.sectionFactor = Math.floor(Math.random() * 7) - 3
				flagData.sectionRotation = chooseRandom(angles)

			// colors
				flagData.primaryHue = chooseRandom(hues)
				flagData.primaryShade = flagData.primaryHue[1]
				flagData.primaryHue = flagData.primaryHue[0]

				flagData.secondaryHue = chooseRandom(hues)
				flagData.secondaryShade = flagData.secondaryHue[1]
				flagData.secondaryHue = flagData.secondaryHue[0]

				flagData.tertiaryHue = Math.floor(Math.random() * 2) ? null : chooseRandom(hues)
				flagData.tertiaryShade = flagData.tertiaryHue ? flagData.tertiaryHue[1] : 0
				flagData.tertiaryHue = flagData.tertiaryHue ? flagData.tertiaryHue[0] : "transparent"

				flagData.quarternaryHue = flagData.tertiaryHue && Math.floor(Math.random() * 4) ? null : chooseRandom(hues)
				flagData.quarternaryShade = flagData.quarternaryHue ? flagData.quarternaryHue[1] : 0
				flagData.quarternaryHue = flagData.quarternaryHue ? flagData.quarternaryHue[0] : "transparent"

				flagData.quintaryHue = flagData.quarternaryHue && Math.floor(Math.random() * 6) ? null : chooseRandom(hues)
				flagData.quintaryShade = flagData.quintaryHue ? flagData.quintaryHue[1] : 0
				flagData.quintaryHue = flagData.quintaryHue ? flagData.quintaryHue[0] : "transparent"

			// seal
				flagData.seal = Math.floor(Math.random() * 3) ? "none" : chooseRandom(Object.keys(symbols))
				flagData.sealHue = flagData.seal == "none" ? "transparent" : chooseRandom(hues)
				flagData.sealShade = flagData.seal == "none" ? 0 : flagData.sealHue[1]
				flagData.sealHue = flagData.seal == "none" ? "transparent" : flagData.sealHue[0]
				flagData.sealLayers = flagData.seal == "none" ? 0 : Math.floor(Math.random() * 2) + 1
				flagData.sealSize = flagData.seal == "none" ? 0 : Math.floor(Math.random() * 5) * 50 + 50
				flagData.sealRotation = flagData.seal == "none" ? 0 : chooseRandom(angles)
				flagData.sealPositions = flagData.seal == "none" ? [] : position

			// ring
				flagData.ring = Math.floor(Math.random() * 5) ? "none" : chooseRandom(Object.keys(symbols))
				flagData.ringHue = flagData.ring == "none" ? "transparent" : chooseRandom(hues)
				flagData.ringShade = flagData.ring == "none" ? 0 : flagData.ringHue[1]
				flagData.ringHue = flagData.ring == "none" ? "transparent" : flagData.ringHue[0]
				flagData.ringCount = flagData.ring == "none" ? 0 : Math.floor(Math.random() * 20) + 4
				flagData.ringSize = flagData.ring == "none" ? 0 : Math.floor(Math.random() * 3) * 50 + 50
				flagData.ringRadius = flagData.ring == "none" ? 0 : Math.floor(Math.random() * 2) * 50 + 100
				flagData.ringRotation = flagData.ring == "none" ? 0 : chooseRandom(angles)
				flagData.ringPositions = flagData.ring == "none" ? [] : position

			// emblems
				flagData.emblem = Math.floor(Math.random() * 4) ? "none" : chooseRandom(Object.keys(symbols))
				flagData.emblemHue = flagData.emblem == "none" ? "transparent" : chooseRandom(hues)
				flagData.emblemShade = flagData.emblem == "none" ? 0 : flagData.emblemHue[1]
				flagData.emblemHue = flagData.emblem == "none" ? "transparent" : flagData.emblemHue[0]
				flagData.emblemSize = flagData.emblem == "none" ? 0 : Math.floor(Math.random() * 4) * 50 + 50
				flagData.emblemRotation = flagData.emblem == "none" ? 0 : chooseRandom(angles)
				flagData.emblemPositions = flagData.emblem == "none" ? [] : position

			// next steps
				if ((flagData.structure == "solid" ||  flagData.secondaryHue == flagData.primaryHue || flagData.sectionCount == 1 || flagData.secondaryHue == "transparent")
				&& ((flagData.seal      == "none") || (flagData.sealHue      == flagData.primaryHue && flagData.sectionCount == 1))
				&& ((flagData.ring      == "none") || (flagData.ringHue      == flagData.primaryHue && flagData.sectionCount == 1))
				&& ((flagData.emblem    == "none") || (flagData.emblemHue    == flagData.primaryHue && flagData.sectionCount == 1))) {
					randomizeData()
				}
				else {
					updateMenu()
	...
	/* updateData */
		function updateData() {
			// field
				flagData.fieldHue           =        document.getElementById("fieldHueSelect").value
				flagData.fieldShade         = Number(document.getElementById("fieldShadeInput").value)

			// structure
				flagData.structure          =        document.getElementById("structureSelect").value
				flagData.sectionCount       = Number(document.getElementById("sectionCountInput").value)
				flagData.sectionFactor      = Number(document.getElementById("sectionFactorInput").value)
				flagData.sectionRotation    = Number(document.getElementById("sectionRotationInput").value)

				flagData.primaryHue         =        document.getElementById("primaryHueSelect").value
				flagData.primaryShade       = Number(document.getElementById("primaryShadeInput").value)
				flagData.secondaryHue       =        document.getElementById("secondaryHueSelect").value
				flagData.secondaryShade     = Number(document.getElementById("secondaryShadeInput").value)
				flagData.tertiaryHue        =        document.getElementById("tertiaryHueSelect").value
				flagData.tertiaryShade      = Number(document.getElementById("tertiaryShadeInput").value)
				flagData.quarternaryHue     =        document.getElementById("quarternaryHueSelect").value
				flagData.quarternaryShade   = Number(document.getElementById("quarternaryShadeInput").value)
				flagData.quintaryHue        =        document.getElementById("quintaryHueSelect").value
				flagData.quintaryShade      = Number(document.getElementById("quintaryShadeInput").value)

			// seal
				flagData.seal               =        document.getElementById("sealSelect").value
				flagData.sealHue            =        document.getElementById("sealHueSelect").value
				flagData.sealShade          = Number(document.getElementById("sealShadeInput").value)
				flagData.sealLayers         = Number(document.getElementById("sealLayersInput").value)
				flagData.sealSize           = Number(document.getElementById("sealSizeInput").value)
				flagData.sealRotation       = Number(document.getElementById("sealRotationInput").value)
				flagData.sealPositions      = Array.from(document.querySelectorAll("input[name='sealPositionsCheckbox']:checked")).map(function(element) { return element.value })

			// ring
				flagData.ring               =        document.getElementById("ringSelect").value
				flagData.ringHue            =        document.getElementById("ringHueSelect").value
				flagData.ringShade          = Number(document.getElementById("ringShadeInput").value)
				flagData.ringCount          = Number(document.getElementById("ringCountInput").value)
				flagData.ringSize           = Number(document.getElementById("ringSizeInput").value)
				flagData.ringRadius         = Number(document.getElementById("ringRadiusInput").value)
				flagData.ringRotation       = Number(document.getElementById("ringRotationInput").value)
				flagData.ringPositions      = Array.from(document.querySelectorAll("input[name='ringPositionsCheckbox']:checked")).map(function(element) { return element.value })

			// emblems
				flagData.emblem             =        document.getElementById("emblemSelect").value
				flagData.emblemHue          =        document.getElementById("emblemHueSelect").value
				flagData.emblemShade        = Number(document.getElementById("emblemShadeInput").value)
				flagData.emblemSize         = Number(document.getElementById("emblemSizeInput").value)
				flagData.emblemRotation     = Number(document.getElementById("emblemRotationInput").value)
				flagData.emblemPositions    = Array.from(document.querySelectorAll("input[name='emblemPositionsCheckbox']:checked")).map(function(element) { return element.value })
...
/*** files ***/
	/* saveCode */
		...
		function saveCode() {
			...
			// project
				flagData.project = "flagMaker"

			// package up
				var saveLink = document.createElement("a")
					saveLink.id = "save-link"
					saveLink.setAttribute("href", "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(flagData)))
				...
				if (upload.value && upload.value.length) {
					var reader = new FileReader()
						...
						reader.onload = function(event) {
							try {
								flagData = JSON.parse(String(event.target.result))
								updateMenu()
								createFlag()
							}
			...
			try {
				// json
					if (type == "json") {
						flagData.project = "flagMaker"
						return {
							name: "flagMaker_" + (new Date().getTime()) + ".json",
							type: "json",
				...
				// png
					if (type == "png") {
						return {
							name: "flagMaker_" + (new Date().getTime()) + ".png",
							type: "png",
...
/*** flag ***/
	...
	/* addField */
		function addField(flagData) {
			drawRectangle(0, 0, canvas.width, canvas.height, {color: colors[flagData.fieldHue][flagData.fieldShade]})
		}

	/* addStructure */
		function addStructure(flagData) {
			// colors
				var structureColors = []
				if (flagData.primaryHue !== "transparent")     { structureColors.push(colors[flagData.primaryHue][flagData.primaryShade]) }
				if (flagData.secondaryHue !== "transparent")   { structureColors.push(colors[flagData.secondaryHue][flagData.secondaryShade]) }
				if (flagData.tertiaryHue !== "transparent")    { structureColors.push(colors[flagData.tertiaryHue][flagData.tertiaryShade]) }
				if (flagData.quarternaryHue !== "transparent") { structureColors.push(colors[flagData.quarternaryHue][flagData.quarternaryShade]) }
				if (flagData.quintaryHue !== "transparent")    { structureColors.push(colors[flagData.quintaryHue][flagData.quintaryShade]) }

				var i = 0
				while (structureColors.length < flagData.sectionCount + 7) {
					structureColors.push(structureColors[i])
					i++
				}

			// solid
				if (flagData.structure == "solid") {
					drawRectangle(0, 0, canvas.width, canvas.height, {color: structureColors[0]})
				}

			// layers
				else if (flagData.structure == "rings") {
					rotateCanvas(canvas.width / 2, canvas.height / 2, flagData.sectionRotation, function() {
						for (var i = 0; i < flagData.sectionCount; i++) {
							drawCircle(canvas.width  / 2,
								       canvas.height / 2,
								(canvas.height * ((flagData.sectionCount - i) / (flagData.sectionCount))) * (1 + flagData.sectionFactor / 10),
								{color: structureColors[i]})
						}
					})
				}

				else if (["square", "diamond", "x", "cross", "jack"].includes(flagData.structure)) {
					rotateCanvas(canvas.width / 2, canvas.height / 2, flagData.sectionRotation, function() {
						for (var i = 0; i < flagData.sectionCount; i++) {
							drawShape(i / flagData.sectionCount * canvas.width  / 2 * (1 + flagData.sectionFactor / 10) - canvas.width  * (flagData.sectionFactor / 10) / 2,
								      i / flagData.sectionCount * canvas.height / 2 * (1 + flagData.sectionFactor / 10) - canvas.height * (flagData.sectionFactor / 10) / 2,
								(canvas.width * ((flagData.sectionCount - i) / (flagData.sectionCount))) * (1 + flagData.sectionFactor / 10),
								(canvas.height * ((flagData.sectionCount - i) / (flagData.sectionCount))) * (1 + flagData.sectionFactor / 10),
								{coordinates: symbols[flagData.structure], color: structureColors[i]})
						}
					})
				}

			// checkers
				else if (flagData.structure == "checkers") {
					rotateCanvas(canvas.width / 2, canvas.height / 2, flagData.sectionRotation, function() {
						var checkerCount = flagData.sectionCount < 4 ? 1 : flagData.sectionCount < 9 ? 4 : flagData.sectionCount < 16 ? 9 : flagData.sectionCount < 25 ? 16 : flagData.sectionCount < 36 ? 25 : flagData.sectionCount < 49 ? 36 : 49
						...
						for (var y = 0; y < sideCount; y++) {
							for (var x = 0; x < sideCount; x++) {
								drawRectangle(width * x * (1 + flagData.sectionFactor / 10) - canvas.width  * (flagData.sectionFactor / 10) / 2,
									         height * y * (1 + flagData.sectionFactor / 10) - canvas.height * (flagData.sectionFactor / 10) / 2,
									width  * (1 + flagData.sectionFactor / 10),
									height * (1 + flagData.sectionFactor / 10),
									{color: structureColors[i]})
								i++
							}
			...
			// stripes
				else if (flagData.structure == "horizontal-stripes") {
					rotateCanvas(canvas.width / 2, canvas.height / 2, flagData.sectionRotation, function() {
						var i = 0
						var count = 0
						while (count < flagData.sectionCount) {
							drawRectangle(-canvas.width,
								canvas.height - ((i + 1) * canvas.height / flagData.sectionCount) - canvas.height * (flagData.sectionFactor / 10) / flagData.sectionCount / 2,
								3 * canvas.width,
								canvas.height / flagData.sectionCount * (1 + flagData.sectionFactor / 10),
								{color: structureColors[i]})

							count++
							i = (count % 2 == 0) ? (count / 2) : flagData.sectionCount - ((count + 1) / 2)
						}
					})
				}

				else if (flagData.structure == "vertical-stripes") {
					rotateCanvas(canvas.width / 2, canvas.height / 2, flagData.sectionRotation, function() {
						var i = 0
						var count = 0
						while (count < flagData.sectionCount) {
							drawRectangle((i * canvas.width / flagData.sectionCount) - canvas.width * (flagData.sectionFactor / 10) / flagData.sectionCount / 2,
								-canvas.height,
								canvas.width / flagData.sectionCount * (1 + flagData.sectionFactor / 10),
								3 * canvas.height,
								{color: structureColors[i]})
				...
				else if (flagData.structure == "wedge-stripes") {
					rotateCanvas(canvas.width / 2, canvas.height / 2, flagData.sectionRotation, function() {
						for (var i = 0; i < flagData.sectionCount; i++) {
							drawCircle(canvas.width  / 2,
								       canvas.height / 2,
								canvas.width * (11 + flagData.sectionFactor) / 20,
								{color: structureColors[i], start: (i + 0.5) * 2 * Math.PI / flagData.sectionCount, end: (i - 0.5) * 2 * Math.PI / flagData.sectionCount})
						}
					})
				}
		}
	...
	/* addSeals */
		function addSeals(flagData) {
			if (flagData.seal !== "none") {
				for (var i in flagData.sealPositions) {
					var x = Number(flagData.sealPositions[i].split(",")[0]) * (canvas.width  / 9) + (canvas.width  / 18)
					var y = Number(flagData.sealPositions[i].split(",")[1]) * (canvas.height / 9) + (canvas.height / 18)

					rotateCanvas(x, canvas.height - y, flagData.sealRotation, function() {
						for (var l = 1; l <= flagData.sealLayers; l++) {
							var color = (l % 2 == 0) ? colors[flagData.fieldHue][flagData.fieldShade] : colors[flagData.sealHue][flagData.sealShade]
							if (flagData.seal == "circle") {
								drawCircle(x, y, (flagData.sealSize / 2) * (5 - l) / 5, {color: color})
							}
							else if (flagData.seal == "ring") {
								drawCircle(x, y, (flagData.sealSize / 2) * (5 - l) / 5, {color: color, border: flagData.sealSize / 10})
							}
							else {
								drawShape(x - (flagData.sealSize / 2 * (5 - l) / 5), canvas.height - (y + (flagData.sealSize / 2 * (5 - l) / 5)), flagData.sealSize * (5 - l) / 5, flagData.sealSize * (5 - l) / 5, {color: color, coordinates: symbols[flagData.seal]})
							}
						}
					})
				}
	...
	/* addRing */
		function addRing(flagData) {
			if (flagData.ring !== "none") {
				for (var i in flagData.ringPositions) {
					var x = Number(flagData.ringPositions[i].split(",")[0]) * (canvas.width  / 9) + (canvas.width  / 18)
					var y = Number(flagData.ringPositions[i].split(",")[1]) * (canvas.height / 9) + (canvas.height / 18)
					var color = colors[flagData.ringHue][flagData.ringShade]

					rotateCanvas(x, canvas.height - y, flagData.ringRotation, function() {
						for (var i = 1; i <= flagData.ringCount; i++) {
							var rotation = 360 / flagData.ringCount * i

								context.translate(-(flagData.ringRadius / 2), -(flagData.ringRadius / 2))
								rotateCanvas(x + (flagData.ringRadius / 2), canvas.height - y + (flagData.ringRadius / 2), rotation, function() {
									rotateCanvas(x, canvas.height - y, 360 / flagData.ringCount, function() {
										if (flagData.ring == "circle") {
											drawCircle(x, y, (flagData.ringSize / 2), {color: color})
										}
										else if (flagData.ring == "ring") {
											drawCircle(x, y, (flagData.ringSize / 2), {color: color, border: flagData.ringSize / 10})
										}
										else {
											drawShape(x - (flagData.ringSize / 2), canvas.height - (y + (flagData.ringSize / 2)), flagData.ringSize, flagData.ringSize, {color: color, coordinates: symbols[flagData.ring]})
										}
									})
								})
								context.translate((flagData.ringRadius / 2), (flagData.ringRadius / 2))
	...
	/* addEmblems */
		function addEmblems(flagData) {
			if (flagData.emblem !== "none") {
				for (var i in flagData.emblemPositions) {
					var x = Number(flagData.emblemPositions[i].split(",")[0]) * (canvas.width  / 9) + (canvas.width  / 18)
					var y = Number(flagData.emblemPositions[i].split(",")[1]) * (canvas.height / 9) + (canvas.height / 18)
					var color = colors[flagData.emblemHue][flagData.emblemShade]

					rotateCanvas(x, canvas.height - y, flagData.emblemRotation, function() {
						if (flagData.emblem == "circle") {
							drawCircle(x, y, (flagData.emblemSize / 2), {color: color})
						}
						else if (flagData.emblem == "ring") {
							drawCircle(x, y, (flagData.emblemSize / 2), {color: color, border: flagData.emblemSize / 10})
						}
						else {
							drawShape(x - (flagData.emblemSize / 2), canvas.height - (y + (flagData.emblemSize / 2)), flagData.emblemSize, flagData.emblemSize, {color: color, coordinates: symbols[flagData.emblem]})
						}
					})
				}
			}
